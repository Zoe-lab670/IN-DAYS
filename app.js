...
// it does not attempt to mark make-up workdays as holidays.
const HOLIDAY_2026 = [
  {start:'2026-01-01',end:'2026-01-03',name:'元旦',ids:['newyear','newyear_firework']},
  {start:'2026-02-15',end:'2026-02-23',name:'春节',ids:['spring_lantern','red_envelope','dumpling']},
  {start:'2026-04-04',end:'2026-04-06',name:'清明节',ids:['qingming_kite','qingming_willow']},
  {start:'2026-05-01',end:'2026-05-05',name:'劳动节',ids:['mayday_tools','mayday_luggage']},
  {start:'2026-06-19',end:'2026-06-21',name:'端午节',ids:['dragonboat','zongzi','mugwort']},
  {start:'2026-09-25',end:'2026-09-27',name:'中秋节',ids:['mooncake','fullmoon','midautumn_lantern','moonrabbit']},
  {start:'2026-10-01',end:'2026-10-07',name:'国庆节',ids:['nationalflag','nationalfirework','nationalribbon']}
];
...
if(STICKERS.length!==233){console.error('[IN DAYS] sticker library integrity check failed:',STICKERS.length);}

const EXPECTED_STICKER_COUNT=233;
if(STICKERS.length!==EXPECTED_STICKER_COUNT){console.warn(`[IN DAYS] sticker count mismatch: expected ${EXPECTED_STICKER_COUNT}, got ${STICKERS.length}`);}
...
const CANVAS_TEMPLATES = {
  free:{label:'自由留白',desc:'保留现在的自由摆放方式'},
  focus:{label:'主角居中',desc:'标题与正文集中，贴纸围绕主角'},
  split:{label:'左右分栏',desc:'左侧文字，右侧贴纸'},
  gallery:{label:'小画廊',desc:'照片/贴纸组成轻量拼贴'}
};
const STICKER_SCENES = {
  tea:{label:'下午茶',desc:'茶杯 + 小点心 + 花',ids:['tea','cake','flower'],positions:[[.67,.44,1.05,-4],[.80,.58,.78,5],[.62,.69,.68,-3]]},
  work:{label:'工作一角',desc:'电脑 + 咖啡 + 笔',ids:['laptop','coffee','pen'],positions:[[.72,.45,1.02,-2],[.58,.63,.74,4],[.82,.70,.68,-5]]},
  weekend:{label:'周末散步',desc:'太阳 + 树 + 小动物',ids:['sun','tree','dog'],positions:[[.69,.38,.72,-3],[.80,.67,.88,3],[.58,.72,.66,-4]]}
};
function applyCanvasTemplate(key){
  const e=state.entry;if(!e||!CANVAS_TEMPLATES[key])return;
  const before=clone(e);e.layoutTemplate=key;
  if(key==='focus'){
    e.titlePos={x:.16,y:.28};e.contentPos={x:.16,y:.46};
    (e.stickers||[]).forEach((it,i)=>{if(it.locked)return;const a=[[.70,.40],[.82,.48],[.66,.64],[.84,.70]][i%4];it.x=a[0];it.y=a[1];});
  }else if(key==='split'){
    e.titlePos={x:.16,y:.28};e.contentPos={x:.16,y:.50};
    (e.stickers||[]).forEach((it,i)=>{if(it.locked)return;it.x=.70+(i%2)*.16;it.y=.38+Math.floor(i/2)*.15;});
  }else if(key==='gallery'){
    e.titlePos={x:.16,y:.27};e.contentPos={x:.16,y:.44};
...
function constrainCanvasElementPosition(kind,item,x,y){
  ...
  const overlapsMood=nx-halfW < MOOD_SAFE_ZONE.right && nx+halfW > MOOD_SAFE_ZONE.left && ny-halfH < MOOD_SAFE_ZONE.bottom && ny+halfH > MOOD_SAFE_ZONE.top;
  if(overlapsMood){
    const pushLeft=MOOD_SAFE_ZONE.left-halfW-MOOD_SAFE_ZONE.margin;
    const pushDown=MOOD_SAFE_ZONE.bottom+halfH+MOOD_SAFE_ZONE.margin;
    if(pushLeft-halfW >= margin) nx=pushLeft; else ny=pushDown;
    nx=clamp(nx,halfW+margin,1-halfW-margin); ny=clamp(ny,halfH+margin,1-halfH-margin);
  }
  return {x:nx,y:ny};
}
...
function normalizePhotoItems(items){
  ...
  const normalized=items.map((p,i)=>({
    ...
    src:String(p?.src||''),
    x:clamp(Number(p?.x)||(.48+(i%3)*.12),.08,.92),
    y:clamp(Number(p?.y)||(.46+(i%3)*.10),.20,.90),
    scale:clamp(Number(p?.scale)||1,.35,1.8),
    rotation:clamp(Number(p?.rotation)||0,-180,180),
    z:Number.isFinite(Number(p?.z))?Number(p.z):i+1,
    locked:!!p?.locked,
    ratio:clamp(Number(p?.ratio)||4/3,.55,2.2),
...
function normalizeDesign(raw,date){
  ...
  const result={...base,
    ...
    layoutTemplate:['free','focus','split','gallery'].includes(raw.layoutTemplate)?raw.layoutTemplate:'free',
...
function mergeRow(row){
  ...
  const rawObject=!!raw && typeof raw==='object' && !legacyArray;
  const merged={
    ...row,
    ...design,
    title: legacyArray
      ? String(row.title||'')
      : String((rawObject && String(raw.title||'').trim()) ? raw.title : (row.title||design.title||'')),
    content: legacyArray
      ? String(row.content||'')
      : String((rawObject && String(raw.content||'').trim()) ? raw.content : (row.content||design.content||'')),
    background: legacyArray
      ? String(row.background||'#fffdf7')
      : String((rawObject && raw.background) ? raw.background : (row.background||design.background||'#fffdf7')),
    stickers: design.stickers,
    photos: design.photos
  };
  return merged;
}
function normalizeLayerOrder(stickers,photos){
  const layers=[...(stickers||[]).map((item,index)=>({item,index,z:Number(item?.z)||0,kind:'sticker'})),...(photos||[]).map((item,index)=>({item,index,z:Number(item?.z)||0,kind:'photo'}))];
  layers.sort((a,b)=>a.z-b.z || a.index-b.index || (a.kind==='photo' ? -1 : 1));
  layers.forEach((layer,index)=>{layer.item.z=index+1;});
  return {stickers,photos};
}
...
async function loadEntriesInRange(startDate,endDate){
  ...
  const out={};const start=parseKey(startDate),end=parseKey(endDate);if(start.y===end.y&&start.m===end.m){for(let d=start.d;d<=end.d;d++){const k=dateKey(start.y,start.m,d),e=chooseWithDraft(mergeRow(getLocal(k)),k);if(e)out[k]=e;}return out;}
  let cursor=new Date(start.y,start.m,start.d);const endLocal=new Date(end.y,end.m,end.d);while(cursor<=endLocal){const k=dateKey(cursor.getFullYear(),cursor.getMonth(),cursor.getDate()),e=chooseWithDraft(mergeRow(getLocal(k)),k);if(e)out[k]=e;cursor.setDate(cursor.getDate()+1);}return out;
}
async function loadSelectedEntry(){
  ...
  renderCalendar();updateSidebarNav();if(state.drawerOpen)renderDrawer();checkCapsule();
}
...
function timelineDayHtml(e,key,day,active,today){const mood=MOODS.find(m=>m.key===e.mood);return `<button class="timeline-card ${active?'active':''}" data-day="${day}"><div class="timeline-date"><b>${String(day).padStart(2,'0')}</b><span>${today?'TODAY':monthCN[state.month]}</span></div><div class="timeline-dot"></div><div class="timeline-thumb" style="--day-bg:${e.background||'#fffdf7'}"><div class="mini-canvas" style="background:${e.background||'#fffdf7'}">${miniObjectHtml(e)}</div></div><div class="timeline-text"><div class="timeline-meta">${mood?`<span class="timeline-mood">${escapeHtml(mood.label)}</span>`:''}${e.stickers?.length?`<span>${e.stickers.length} 枚贴纸</span>`:''}</div><strong>${escapeHtml(e.title||'这一天')}</strong><p>${escapeHtml(e.content||'')}</p></div></button>`;}
  ...
  root.innerHTML=`<section class="monthly-report" aria-label="MONTHLY REPORT">
    ...
    <div class="report-body">
      ...
      <div class="report-stats">
        ...
        <button class="report-featured" id="insightTopSticker"><span>常用贴纸</span><div class="featured-inner">${top?svgSticker(top.st,32):'<span class="insight-placeholder">♡</span>'}<div><b>${top?escapeHtml(top.st.name):'还没有'}</b><small>${top?`${top.count} 次出现`:'之后再慢慢填满它'}</small></div></div></button>
      </div>
    </div>
...
function renderShell(){
  app.innerHTML=`<svg aria-hidden="true" width="0" height="0" style="position:absolute;overflow:hidden"><defs><filter id="stickerRough" x="-8%" y="-8%" width="116%" height="116%"><feTurbulence type="fractalNoise" baseFrequency="0.035" numOctaves="1" seed="7" result="noise"/><feDisplacementMap in="SourceGraphic" in2="noise" scale="0.55" xChannelSelector="R" yChannelSelector="G"/></filter></defs></svg><main class="app-shell">
    <aside class="workspace-sidebar" aria-label="IN DAYS 导航">
      ...
      <nav class="sidebar-nav">
        ...
        <button class="sidebar-nav-item" data-nav="mosaic"><span>▦</span><b>画册</b></button>
        <button class="sidebar-nav-item" data-nav="timeline"><span>◷</span><b>时间轴</b></button>
        <button class="sidebar-nav-item" data-nav="review"><span>☆</span><b>本月回顾</b></button>
        <button class="sidebar-nav-item" data-nav="annual"><span>◌</span><b>年度回顾</b></button>
        <button class="sidebar-nav-item" data-nav="export"><span>↓</span><b>导出本月</b></button>
      </nav>
      <div class="sidebar-note"><div class="sidebar-doodle">⌁</div><p>在一隅的时光里，<br/>把平凡的日子<br/>过成喜欢的样子。♡</p></div>
      <div class="sidebar-spacer"></div>
      <button class="sidebar-mood" id="sidebarMoodBtn"><span class="sidebar-mood-dot">◡</span><span><small>今日心情</small><strong id="sidebarMoodLabel">未设置</strong></span><i>›</i></button>
      <div class="sidebar-user" id="sidebarUser"><span class="sidebar-user-avatar">一</span><span>我的一隅</span></div>
    </aside>
    <section class="workspace-main">
      <header class="topbar"><div class="topbar-month-caption"><span class="eyebrow">A LITTLE CORNER, EVERY DAY</span></div><div class="top-actions"><button class="today-button" id="globalSearchBtn">搜索记录</button><button class="today-button" id="todayBtn">今天</button><span id="accountArea"></span></div></header>
  ...
  </main><div id="drawerRoot"></div><div id="authRoot"></div><div id="profileRoot"></div><div id="reviewRoot"></div><div id="annualReviewRoot"></div><div id="searchRoot"></div><div id="capsuleRoot"></div><div id="exportMonthRoot"></div><div class="toast" id="toast"></div>`;
  ...
  document.querySelectorAll('.view-mode-btn').forEach(b=>b.addEventListener('click',()=>{state.viewMode=b.dataset.view;localStorage.setItem('in-days:view',state.viewMode);renderCalendar();updateSidebarNav();}));
  document.querySelectorAll('.sidebar-nav-item').forEach(b=>b.addEventListener('click',()=>{const nav=b.dataset.nav;if(nav==='home'||nav==='month'){state.viewMode='month';localStorage.setItem('in-days:view','month');renderCalendar();}else if(nav==='mosaic'){state.viewMode='mosaic';localStorage.setItem('in-days:view','mosaic');renderCalendar();}else if(nav==='timeline'){state.viewMode='timeline';localStorage.setItem('in-days:view','timeline');renderCalendar();}else if(nav==='review'){openReview();}else if(nav==='annual'){openAnnualReview();}else if(nav==='export'){openMonthExport();}updateSidebarNav();}));
  document.getElementById('sidebarMoodBtn').addEventListener('click',()=>{openDrawer();setTimeout(()=>document.querySelector('.mood-choice')?.scrollIntoView({behavior:'smooth',block:'center'}),80);});
  renderAccount();
  updateSidebarNav();
}
function updateSidebarNav(){document.querySelectorAll('.sidebar-nav-item').forEach(b=>{const nav=b.dataset.nav;const active=(nav==='month'&&state.viewMode==='month')||(nav==='mosaic'&&state.viewMode==='mosaic')||(nav==='timeline'&&state.viewMode==='timeline');b.classList.toggle('active',active);});const e=state.entry||emptyEntry(currentDateKey());const mood=MOODS.find(m=>m.key===e.mood);const label=document.getElementById('sidebarMoodLabel');if(label)label.textContent=mood?.label||'未设置';const user=document.getElementById('sidebarUser');if(user&&state.user)user.querySelector('span:nth-child(2)').textContent=getDisplayName(state.user);}
...
function collectLocalDrafts(){const drafts={};for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i)||'';if(k.startsWith(DRAFT_PREFIX)){try{drafts[k.slice(DRAFT_PREFIX.length)]=JSON.parse(localStorage.getItem(k));}catch{}}}return drafts;}
async function fetchAllCloudEntries(){if(!supabase||!state.user)throw new Error('请先登录后再备份云端数据。');const {data,error}=await supabase.from('journal_entries').select('id,user_id,entry_date,title,content,background,stickers,updated_at').eq('user_id',state.user.id).order('entry_date',{ascending:true});if(error)throw error;return data||[];}
function downloadJsonFile(payload,filename){const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json;charset=utf-8'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=filename;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
async function exportFullBackup(){const st=document.getElementById('profileStatus');try{st.textContent='准备备份…';const cloudEntries=supabase&&state.user?await fetchAllCloudEntries():Object.values(state.monthEntries||{});const backup={schema:'in-days-backup',version:1,exported_at:new Date().toISOString(),app:'一隅｜IN DAYS',user_id:state.user?.id||null,entries:cloudEntries.map(r=>({id:r.id||null,user_id:r.user_id||null,entry_date:r.entry_date,title:r.title||'',content:r.content||'',background:r.background||'#fffdf7',stickers:r.stickers||{},updated_at:r.updated_at||null})),drafts:collectLocalDrafts(),prefs:{favorites:[...state.favorites],recent:[...state.recent]},local_view:localStorage.getItem('in-days:view')||'month'};downloadJsonFile(backup,`in-days-backup-${new Date().toISOString().slice(0,10)}.json`);st.textContent='备份已导出。';toast('完整备份已导出');}catch(e){st.textContent=e.message||'备份失败';toast(st.textContent);}}
async function handleBackupImport(ev){const input=ev.target,st=document.getElementById('profileStatus'),file=input?.files?.[0];input.value='';if(!file)return;try{const raw=await file.text(),backup=JSON.parse(raw);if(backup?.schema!=='in-days-backup'||!Array.isArray(backup.entries))throw new Error('这不是有效的一隅备份文件。');if(!confirm(`将恢复 ${backup.entries.length} 条记录。已有同日期记录会被覆盖，是否继续？`))return;st.textContent='恢复中…';if(supabase){if(!state.user)throw new Error('请先登录后再恢复云端数据。');const userId=await currentUserId();if(!userId)throw new Error('登录状态已失效，请重新登录。');const rows=backup.entries.map(r=>{const e=normalizeDesign(r.stickers&&typeof r.stickers==='object'?{...r.stickers,title:r.title||r.stickers.title||'',content:r.content||r.stickers.content||'',background:r.background||r.stickers.background||'#fffdf7'}:{title:r.title||'',content:r.content||'',background:r.background||'#fffdf7'},r.entry_date);return {user_id:userId,entry_date:r.entry_date,title:e.title||'',content:e.content||'',background:e.background||'#fffdf7',stickers:e,updated_at:new Date().toISOString()};});for(let i=0;i<rows.length;i+=50){const {error}=await supabase.from('journal_entries').upsert(rows.slice(i,i+50),{onConflict:'user_id,entry_date'});if(error)throw error;}}else{for(const r of backup.entries){const e=normalizeDesign(r.stickers&&typeof r.stickers==='object'?{...r.stickers,title:r.title||r.stickers.title||'',content:r.content||r.stickers.content||'',background:r.background||r.stickers.background||'#fffdf7'}:{title:r.title||'',content:r.content||'',background:r.background||'#fffdf7'},r.entry_date);setLocal({...e,entry_date:r.entry_date});}}for(const [k,v] of Object.entries(backup.drafts||{})){try{localStorage.setItem(DRAFT_PREFIX+k,JSON.stringify(v));}catch{}}if(Array.isArray(backup.prefs?.favorites))state.favorites=new Set(backup.prefs.favorites);if(Array.isArray(backup.prefs?.recent))state.recent=backup.prefs.recent.slice(0,24);persistPrefs();await loadCurrentMonth();await loadSelectedEntry();st.textContent='恢复完成。';toast('备份已恢复');}catch(e){st.textContent=e.message||'恢复失败';toast(st.textContent);}}
...
async function initAuth(){await ensureSupabaseClient();renderAccount();if(!supabase)return;const {data}=await supabase.auth.getSession();state.user=data.session?.user||null;renderAccount();await refreshAfterAuth();supabase.auth.onAuthStateChange((_e,session)=>{state.user=session?.user||null;renderAccount();if(state.user)closeAuth();setTimeout(()=>{refreshAfterAuth().catch(error=>console.warn('[IN DAYS] auth refresh failed',error));},0);});}
...
function festivalForDate(key){
  if(String(key).startsWith('2026-')){
    const exact=HOLIDAY_2026.find(h=>key>=h.start&&key<=h.end);
    if(exact)return {name:exact.name,ids:exact.ids};
  }
  ...
  if(mm===4 && d>=4 && d<=6)return {name:'清明节',ids:['qingming_kite','qingming_willow']};
...
function recommendationIds(){
  const text=getTextForRecommendations().toLowerCase();const scores=new Map();
  if(!text){
    const fallback=['coffee','tea','book','flower','cat','sun','happy','calm','cake','moon','star','leaf','laptop','camera'];
    const festival=festivalForDate(currentDateKey());
    festival?.ids.forEach(id=>scores.set(id,12));
    [...state.recent,...fallback].forEach((id,i)=>{if(STICKERS.some(s=>s.id===id))scores.set(id,Math.max(scores.get(id)||0,10-Math.min(i,8)));});
    return [...scores.entries()].sort((a,b)=>b[1]-a[1]).slice(0,14).map(([id])=>id);
  }
...
function adjustSelectedScale(delta){const before=clone(state.entry);if(state.selectedText){const key=state.selectedText+'Style',style=state.entry[key]||{};style.fontSize=clamp((style.fontSize|| (state.selectedText==='title'?25:12))+(delta*28),state.selectedText==='title'?16:9,state.selectedText==='title'?54:28);state.entry[key]=style;const pk=state.selectedText+'Pos';state.entry[pk]=constrainTextPosition(state.selectedText,state.entry,state.entry[pk]?.x??.5,state.entry[pk]?.y??.5);}state.entry.stickers.forEach(x=>{if(state.selectedCanvas.has(x.id)&&!x.locked){x.scale=clamp((x.scale||1)+delta,.35,2.15);const q=constrainCanvasElementPosition('sticker',x,x.x,x.y);x.x=q.x;x.y=q.y;}});state.entry.photos.forEach(x=>{if(state.selectedPhoto.has(x.id)&&!x.locked){x.scale=clamp((x.scale||1)+delta,.35,1.8);const q=constrainCanvasElementPosition('photo',x,x.x,x.y);x.x=q.x;x.y=q.y;}});recordChange(before);renderCanvasOnly();}
...
async function compressImage(file){const src=await fileToDataUrl(file);return await new Promise((resolve,reject)=>{const img=new Image();img.onload=()=>{const ratio=clamp(img.naturalWidth/Math.max(1,img.naturalHeight),.55,2.2),max=1000,scale=Math.min(1,max/Math.max(img.naturalWidth,img.naturalHeight)),c=document.createElement('canvas');c.width=Math.max(1,Math.round(img.naturalWidth*scale));c.height=Math.max(1,Math.round(img.naturalHeight*scale));const ctx=c.getContext('2d');ctx.drawImage(img,0,0,c.width,c.height);resolve({src:c.toDataURL('image/jpeg',.76),ratio});};img.onerror=reject;img.src=src;});}
async function addPhoto(file){
  try{
    const prepared=await compressImage(file),before=clone(state.entry),p=findOpenPosition((state.entry.photos||[]).length);
    const maxZ=Math.max(0,...(state.entry.stickers||[]).map(x=>Number(x.z)||0),...(state.entry.photos||[]).map(x=>Number(x.z)||0));const photo={id:uid('photo'),src:prepared.src,ratio:prepared.ratio,frame:'paper',cropMode:'original',objectPositionX:50,objectPositionY:50,x:p.x,y:p.y,scale:1,rotation:0,z:maxZ+1,locked:false};
...
function bindDrawerEvents(){
  ...
  document.querySelectorAll('[data-template]').forEach(btn=>btn.addEventListener('click',()=>applyCanvasTemplate(btn.dataset.template)));
  document.querySelectorAll('[data-scene]').forEach(btn=>btn.addEventListener('click',()=>addStickerScene(btn.dataset.scene)));
...
window.addEventListener('beforeunload',()=>{
  try{if(state.drawerOpen&&state.entry){saveDraft(state.entry);}}catch(error){console.warn('[IN DAYS] beforeunload draft save failed',error);}
});

document.addEventListener('keydown',e=>{
  if(!state.drawerOpen||state.textEdit)return;
  if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='s'){e.preventDefault();commitBeforeNavigation();saveEntry();return;}
  if(e.key==='Escape'){clearSelection();return;}
  if((e.key==='Delete'||e.key==='Backspace')&&(state.selectedCanvas.size||state.selectedPhoto.size)){e.preventDefault();removeSelected();return;}
  if(e.key==='c'&&(e.metaKey||e.ctrlKey)&&(state.selectedCanvas.size||state.selectedPhoto.size)){e.preventDefault();duplicateSelected();return;}
  const step=e.shiftKey ? 0.012 : 0.004;
  if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key)&& (state.selectedText||state.selectedCanvas.size||state.selectedPhoto.size)){
    e.preventDefault();
...
function applySelectedTextStyleToCanvas(){
  if(!state.selectedText)return;
  const kind=state.selectedText;
  const style=state.entry[`${kind}Style`]|| (kind==='title'?DEFAULT_TITLE_STYLE:DEFAULT_CONTENT_STYLE);
  const el=document.querySelector(`.canvas-text[data-drag-kind="${kind}"]`);
  if(!el)return;
  const fam=FONT_OPTIONS[style.fontFamily]?.family||FONT_OPTIONS.system.family;
  el.style.fontFamily=fam;
  el.style.fontSize=`${Number(style.fontSize)||12}px`;
  el.style.fontWeight=String(style.weight||400);
...
function renderTools(){const wrap=document.getElementById('objectTools');if(!wrap)return;let html='';const n=state.selectedCanvas.size+state.selectedPhoto.size;if(n){const locked=[...state.selectedCanvas].some(id=>state.entry.stickers.find(x=>x.id===id)?.locked)||[...state.selectedPhoto].some(id=>state.entry.photos.find(x=>x.id===id)?.locked);html+=`<div class="tool-group"><span class="tool-label">选中 ${n} 个</span><button class="ghost-mini" id="scaleDown">缩小</button><button class="ghost-mini" id="scaleUp">放大</button><button class="ghost-mini" id="rotateLeft">↺</button><button class="ghost-mini" id="rotateRight">↻</button><button class="ghost-mini" id="sendBack">后置</button><button class="ghost-mini" id="bringFront">前置</button><button class="ghost-mini" id="duplicateSelected">复制</button><button class="ghost-mini" id="lockSelected">${locked?'解锁':'锁定'}</button><button class="ghost-mini danger-mini" id="deleteSelected">删除</button></div>`;}
  ...
  const photoTool=document.getElementById('photoTools');if(photoTool){
    if(state.selectedPhoto.size===1){
      ...
      photoTool.innerHTML=`<div class="tool-group photo-edit-tools"><span class="tool-label">照片样式</span><button class="ghost-mini ${frame==='paper'?'active':''}" data-photo-frame="paper">纸张</button><button class="ghost-mini ${frame==='tape'?'active':''}" data-photo-frame="tape">胶带</button><button class="ghost-mini ${frame==='shadow'?'active':''}" data-photo-frame="shadow">阴影</button><button class="ghost-mini ${frame==='none'?'active':''}" data-photo-frame="none">无框</button><span class="tool-label photo-crop-label">裁切</span><button class="ghost-mini ${cropMode==='original'?'active':''}" data-photo-crop="original">原比例</button><button class="ghost-mini ${cropMode==='4:3'?'active':''}" data-photo-crop="4:3">4:3</button></div>`;
      photoTool.querySelectorAll('[data-photo-frame]').forEach(btn=>btn.addEventListener('click',()=>{const before=clone(state.entry);p.frame=btn.dataset.photoFrame;recordChange(before);renderCanvasOnly();renderTools();}));
...
function bindCanvasInteractions(canvas){
  ...
  const move=e=>{
    ...
    if(candidate.kind==='sticker'){
      ...
      const posKey=candidate.kind==='title'?'titlePos':'contentPos',base=candidate.before[posKey]||state.entry[posKey],q=constrainCanvasElementPosition('text',{kind:candidate.kind,entry:state.entry},base.x+dx,base.y+dy);state.entry[posKey]={x:q.x,y:q.y};const el=canvas.querySelector(`[data-drag-kind="${candidate.kind}"]`);if(el){el.style.left=`${state.entry[posKey].x*100}%`;el.style.top=`${state.entry[posKey].y*100}%`;}
    }
...
function setTextSize(kind,delta){const before=clone(state.entry),style=state.entry[`${kind}Style`]||{};style.fontSize=clamp((style.fontSize|| (kind==='title'?25:12))+delta,kind==='title'?16:9,kind==='title'?46:22);state.entry[`${kind}Style`]=style;const pk=kind+'Pos';state.entry[pk]=constrainTextPosition(kind,state.entry,state.entry[pk]?.x??.5,state.entry[pk]?.y??.5);recordChange(before);renderCanvasOnly();renderTools();}
...
function moodScore(key){return ({sad:1,tired:2,anxious:2,calm:3,full:4,happy:5,excited:5,love:5,wow:5}[key]||0);}
function buildMoodTrendSvg(){
  const entries=[]; const total=new Date(state.year,state.month+1,0).getDate();
  for(let d=1;d<=total;d++){const k=dateKey(state.year,state.month,d),e=state.monthEntries[k];if(e?.mood)entries.push({d,score:moodScore(e.mood),label:MOODS.find(m=>m.key===e.mood)?.label||''});}
  if(!entries.length)return '<div class="review-empty-trend">这个月还没有足够的心情记录。</div>';
  const W=640,H=150,pad=18,maxX=Math.max(1,entries.length-1),maxY=5;
  const pts=entries.map((x,i)=>`${pad+i*(W-pad*2)/Math.max(1,maxX)},${H-pad-x.score*(H-pad*2)/maxY}`).join(' ');
  const dots=entries.map((x,i)=>`<circle cx="${pad+i*(W-pad*2)/Math.max(1,maxX)}" cy="${H-pad-x.score*(H-pad*2)/maxY}" r="4" fill="#8da684"/>`).join('');
  return `<div class="review-trend-wrap"><svg class="review-trend" viewBox="0 0 ${W} ${H}" role="img" aria-label="本月心情趋势"><line x1="${pad}" y1="${H-pad}" x2="${W-pad}" y2="${H-pad}" stroke="#e8e4da"/><line x1="${pad}" y1="${H/2}" x2="${W-pad}" y2="${H/2}" stroke="#eeeae1"/><polyline points="${pts}" fill="none" stroke="#8da684" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>${dots}</svg><div class="review-trend-meta"><span>低落</span><span>平稳</span><span>开心</span></div></div>`;
}
function monthlyReviewData(){
  const days=Object.values(state.monthEntries).filter(hasEntryContent),counts={},moodCounts={};
  days.forEach(e=>(e.stickers||[]).forEach(s=>counts[s.stickerId]=(counts[s.stickerId]||0)+1));
  days.forEach(e=>{if(e.mood)moodCounts[e.mood]=(moodCounts[e.mood]||0)+1;});
  const mostMood=Object.entries(moodCounts).sort((a,b)=>b[1]-a[1])[0];
  const holidayDays=Object.keys(state.monthEntries).map(k=>({k,h:holidayCueForDate(k)})).filter(x=>x.h);
  return {days,counts,moodCounts,mostMood,holidayDays};
}
function buildMonthlyReviewSvg(){
  const {days,counts,moodCounts,mostMood,holidayDays}=monthlyReviewData();
  const W=1400,H=1000,bg='#fbfaf6',ink='#3f403b',muted='#8f8c84';
  let svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}"><rect width="100%" height="100%" fill="${bg}"/><rect x="48" y="48" width="${W-96}" height="${H-96}" rx="42" fill="#fffefb" stroke="#ded9ce"/><text x="92" y="112" font-size="18" letter-spacing="5" fill="${muted}">IN DAYS · MONTH REVIEW</text><text x="92" y="172" font-size="58" font-family="-apple-system,BlinkMacSystemFont,'PingFang SC','Microsoft YaHei',sans-serif" fill="${ink}">${state.year} · ${escapeHtml(monthCN[state.month])}</text><text x="92" y="207" font-size="15" fill="${muted}">记录生活，收藏每一个小小的你。</text>`;
  const stats=[['记录日',days.length],['贴纸',days.reduce((n,e)=>n+(e.stickers||[]).length,0)],['照片日',days.filter(e=>e.photos?.length).length]];
  stats.forEach((s,i)=>{const x=92+i*250;svg+=`<rect x="${x}" y="258" width="220" height="118" rx="24" fill="#f6f4ed"/><text x="${x+24}" y="298" font-size="14" fill="${muted}">${s[0]}</text><text x="${x+24}" y="350" font-size="36" fill="${ink}">${s[1]}</text>`;});
  svg+=`<text x="92" y="446" font-size="16" fill="${ink}">本月心情趋势</text>`;
  const trend=buildMoodTrendSvg().match(/<svg[^>]*viewBox="0 0 ([^"]+)">([\s\S]*?)<\/svg>/); if(trend){const inner=trend[2].replace(/#8da684/g,'#8da684').replace(/#e8e4da/g,'#e8e4da').replace(/#eeeae1/g,'#eeeae1');svg+=`<g transform="translate(92 462) scale(.92)">${inner}</g>`;} else svg+=`<text x="92" y="500" font-size="13" fill="${muted}">还没有足够的心情记录。</text>`;
  const top=Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,6);svg+=`<text x="820" y="446" font-size="16" fill="${ink}">最常出现</text>`;top.forEach(([id,n],i)=>{const st=STICKERS.find(x=>x.id===id),y=486+i*60;if(!st)return;svg+=`<g transform="translate(820 ${y})">${exportSvgForSticker(st,24,12,42,1,0)}<text x="58" y="17" font-size="13" fill="${ink}">${escapeHtml(st.name)}</text><text x="260" y="17" font-size="11" fill="${muted}">${n} 次</text></g>`;});
  const moodText=mostMood?MOODS.find(m=>m.key===mostMood[0])?.label:'还没设置心情';svg+=`<text x="92" y="820" font-size="16" fill="${ink}">这个月的你</text><text x="92" y="862" font-size="28" fill="${ink}">${escapeHtml(moodText)} · ${days.length?`留下了 ${days.length} 个小片刻`:'还在慢慢开始'}</text><text x="92" y="908" font-size="14" fill="${muted}">${holidayDays.length?`遇见了 ${holidayDays.map(x=>x.h.name).join('、')}`:'让每一个平凡的日子都有迹可循。'}</text><text x="1030" y="906" font-size="13" fill="${muted}">一隅 · IN DAYS</text></svg>`;
  return svg;
}
async function exportMonthlyReviewPng(){try{await svgToPng(buildMonthlyReviewSvg(),1400,1000,`in-days-${state.year}-${String(state.month+1).padStart(2,'0')}-review.png`);toast('本月生活月报已导出。');}catch{toast('月报导出失败，请稍后重试。');}}
function openReview(){
  const {days,counts,moodCounts,mostMood,holidayDays}=monthlyReviewData();
  const top=Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([id,n])=>{const s=STICKERS.find(x=>x.id===id);return s?`<span class="review-sticker">${svgSticker(s,32)} ${escapeHtml(s.name)} · ${n}</span>`:''}).join('');
  document.getElementById('reviewRoot').innerHTML=`<button class="auth-backdrop" id="reviewBackdrop"></button><section class="review-modal"><button class="close-button" id="closeReview">×</button><div class="section-kicker">MONTH REVIEW</div><div class="review-head-row"><div><h2>${state.year} · ${monthCN[state.month]}</h2><p class="review-sub">这个月，你留下了 ${days.length} 个小片刻。${mostMood?` 最常见的心情是「${escapeHtml(MOODS.find(m=>m.key===mostMood[0])?.label||'')}」。`:''}</p></div><button class="save-button mini" id="exportReviewPng">导出生活月报 PNG</button></div><div class="review-stat-grid"><div><strong>${days.length}</strong><span>有记录的日子</span></div><div><strong>${days.reduce((n,e)=>n+(e.stickers||[]).length,0)}</strong><span>枚贴纸</span></div><div><strong>${days.filter(e=>e.photos?.length).length}</strong><span>有照片的日子</span></div></div><div class="review-section"><h3>本月心情趋势</h3>${buildMoodTrendSvg()}</div><div class="review-section"><h3>常出现的贴纸</h3><div class="review-stickers">${top||'<span class="tool-muted">还没有足够的数据。</span>'}</div></div><div class="review-section"><h3>节日记录</h3><div class="mood-summary">${holidayDays.length?holidayDays.map(x=>`<span>${escapeHtml(x.k.slice(5))} · ${escapeHtml(x.h.name)}</span>`).join(''):'<span>这个月没有系统节日提示</span>'}</div></div><div class="review-section"><h3>这个月的心情</h3><div class="mood-summary">${MOODS.map(m=>`<span>${escapeHtml(m.label)} ${moodCounts[m.key]||0}</span>`).join('')}</div></div></section>`;
  document.getElementById('reviewBackdrop').addEventListener('click',()=>document.getElementById('reviewRoot').innerHTML='');
  document.getElementById('closeReview').addEventListener('click',()=>document.getElementById('reviewRoot').innerHTML='');
  document.getElementById('exportReviewPng').addEventListener('click',exportMonthlyReviewPng);
}
...
function compositionSvg(e,w=1200,h=900){return `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}"><rect width="100%" height="100%" rx="44" fill="${escapeHtml(e.background||'#fffdf7')}"/><rect x="24" y="24" width="${w-48}" height="${h-48}" rx="34" fill="none" stroke="#ded9ce" stroke-dasharray="5 7"/>${compositionElementsSvg(e,w,h)}</svg>`;}
...
function normalizeSearchEntryRow(row){const raw=row?.stickers&&typeof row.stickers==='object'?{...row.stickers,title:row.title||row.stickers.title||'',content:row.content||row.stickers.content||'',background:row.background||row.stickers.background||'#fffdf7'}:{title:row?.title||'',content:row?.content||'',background:row?.background||'#fffdf7'};return normalizeDesign(raw,row?.entry_date||raw.entry_date||currentDateKey());}
async function getAllSearchableEntries(){if(supabase&&state.user){const rows=await fetchAllCloudEntries();return rows.map(normalizeSearchEntryRow).filter(hasEntryContent);}const map=new Map();for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i)||'';if(!k.startsWith(LOCAL_PREFIX))continue;try{const raw=JSON.parse(localStorage.getItem(k));const e=normalizeDesign(raw,raw?.entry_date||k.slice(LOCAL_PREFIX.length));if(hasEntryContent(e))map.set(e.entry_date,e);}catch{}}Object.values(state.monthEntries||{}).filter(hasEntryContent).forEach(e=>map.set(e.entry_date,e));return [...map.values()].sort((a,b)=>String(a.entry_date).localeCompare(String(b.entry_date)));}
function searchableStickerNames(e){return (e?.stickers||[]).map(x=>STICKERS.find(s=>s.id===x.stickerId)?.name||'').filter(Boolean).join(' ');}
function scoreEntryMatch(e,q){const title=String(e.title||'').toLowerCase(),content=String(e.content||'').toLowerCase(),stickers=searchableStickerNames(e).toLowerCase(),date=String(e.entry_date||'').toLowerCase();let score=0;for(const t of q.toLowerCase().split(/\s+/).filter(Boolean)){if(title.includes(t))score+=40;if(content.includes(t))score+=25;if(stickers.includes(t))score+=18;if(date.includes(t))score+=15;if((title+' '+content+' '+stickers+' '+date).includes(t))score+=2;}return score;}
function formatSearchDate(key){const m=parseKey(key);return `${m.y}年${m.m+1}月${m.d}日`;}
async function openGlobalSearch(){const root=document.getElementById('searchRoot');if(!root)return;root.innerHTML=`<button class="search-backdrop" id="searchBackdrop"></button><section class="search-modal" role="dialog" aria-modal="true"><button class="close-button" id="closeGlobalSearch">×</button><div class="section-kicker">SEARCH YOUR DAYS</div><h2>找回某一个日子</h2><p class="search-hint">搜索标题、正文、日期或贴纸名称。</p><div class="global-search-input"><span>⌕</span><input id="globalSearchInput" placeholder="例如：旅行 / 咖啡 / 2026-09 / 开心" autocomplete="off" /></div><div id="globalSearchResults" class="global-search-results"><div class="search-empty">输入关键词开始搜索。</div></div></section>`;const close=()=>root.innerHTML='';document.getElementById('searchBackdrop').addEventListener('click',close);document.getElementById('closeGlobalSearch').addEventListener('click',close);const input=document.getElementById('globalSearchInput');let timer=0;input.addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(()=>runGlobalSearch(input.value),120);});input.focus();}
async function runGlobalSearch(query){const root=document.getElementById('globalSearchResults');if(!root)return;const q=String(query||'').trim();if(!q){root.innerHTML='<div class="search-empty">输入关键词开始搜索。</div>';return;}root.innerHTML='<div class="search-loading">正在整理你的记录…</div>';try{const entries=await getAllSearchableEntries();const ranked=entries.map(e=>({e,score:scoreEntryMatch(e,q)})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score||String(b.e.entry_date).localeCompare(String(a.e.entry_date))).slice(0,30);if(!ranked.length){root.innerHTML='<div class="search-empty">没有找到相关记录。换个词试试，比如“海边 / 工作 / 生日”。</div>';return;}root.innerHTML=ranked.map(({e})=>{const mood=MOODS.find(m=>m.key===e.mood);return `<button class="global-search-result" data-date="${escapeHtml(e.entry_date)}"><span class="search-result-date">${escapeHtml(formatSearchDate(e.entry_date))}</span><span class="search-result-main"><strong>${escapeHtml(e.title||'这一天')}</strong><small>${escapeHtml((e.content||'').slice(0,90)||'没有正文')}</small><em>${mood?escapeHtml(mood.label):''}${e.stickers?.length?` · ${e.stickers.length}枚贴纸`:''}</em></span><span class="search-result-arrow">↗</span></button>`;}).join('');root.querySelectorAll('.global-search-result').forEach(btn=>btn.addEventListener('click',async()=>{const m=parseKey(btn.dataset.date);state.year=m.y;state.month=m.m;state.selectedDay=m.d;state.viewMode='month';localStorage.setItem('in-days:view','month');document.getElementById('searchRoot').innerHTML='';renderCalendar();await loadCurrentMonth();await loadSelectedEntry();openDrawer();}));}catch(e){root.innerHTML=`<div class="search-empty">搜索失败：${escapeHtml(e.message||'请稍后重试。')}</div>`;}}
async function getYearEntries(year=state.year){const start=`${year}-01-01`,end=`${year}-12-31`;if(supabase&&state.user){const rows=await loadEntriesInRange(start,end);return Object.values(rows).filter(hasEntryContent).sort((a,b)=>a.entry_date.localeCompare(b.entry_date));}const map=new Map();for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i)||'';if(!k.startsWith(LOCAL_PREFIX))continue;const key=k.slice(LOCAL_PREFIX.length);if(!key.startsWith(String(year)+'-'))continue;try{const raw=JSON.parse(localStorage.getItem(k));const e=normalizeDesign(raw,key);if(hasEntryContent(e))map.set(e.entry_date,e);}catch{}}Object.values(state.monthEntries||{}).filter(e=>hasEntryContent(e)&&String(e.entry_date||'').startsWith(String(year)+'-')).forEach(e=>map.set(e.entry_date,e));return [...map.values()].sort((a,b)=>a.entry_date.localeCompare(b.entry_date));}
function annualReviewData(entries){const monthStats=Array.from({length:12},(_,m)=>({month:m,days:0,stickers:0,photos:0})),moodCounts={},stickerCounts={};entries.forEach(e=>{const m=parseKey(e.entry_date).m,d=monthStats[m];d.days++;d.stickers+=(e.stickers||[]).length;d.photos+=(e.photos||[]).length;if(e.mood)moodCounts[e.mood]=(moodCounts[e.mood]||0)+1;(e.stickers||[]).forEach(x=>{stickerCounts[x.stickerId]=(stickerCounts[x.stickerId]||0)+1;});});const topMood=Object.entries(moodCounts).sort((a,b)=>b[1]-a[1])[0],topStickers=Object.entries(stickerCounts).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([id,n])=>({sticker:STICKERS.find(s=>s.id===id),n})).filter(x=>x.sticker);return{entries,monthStats,moodCounts,topMood,topStickers};}
function annualSummarySentence(d){const mood=d.topMood?MOODS.find(m=>m.key===d.topMood[0])?.label:'';return d.entries.length?`这一年留下了 ${d.entries.length} 个小片刻，${mood?`最常见的心情是「${mood}」。`:'每一种心情都被好好收下。'}`:'这一年还没有留下记录，等一个你想记住的日子。';}
async function openAnnualReview(){const root=document.getElementById('annualReviewRoot');if(!root)return;root.innerHTML='<div class="annual-loading"><div>正在整理这一年的一隅…</div></div>';try{const d=annualReviewData(await getYearEntries(state.year));root.innerHTML=`<button class="auth-backdrop" id="annualBackdrop"></button><section class="annual-modal" role="dialog" aria-modal="true"><button class="close-button" id="closeAnnual">×</button><div class="section-kicker">YEAR REVIEW</div><div class="annual-head"><div><h2>${state.year} · 年度回顾</h2><p>${escapeHtml(annualSummarySentence(d))}</p></div><div class="annual-actions"><button class="subtle-upload" id="exportAnnualPng">年度回顾 PNG</button><button class="save-button mini" id="exportAnnualJson">年度数据 JSON</button></div></div><div class="annual-stat-grid"><div><strong>${d.entries.length}</strong><span>有记录的日子</span></div><div><strong>${d.entries.reduce((n,e)=>n+(e.stickers||[]).length,0)}</strong><span>枚贴纸</span></div><div><strong>${d.entries.filter(e=>e.photos?.length).length}</strong><span>有照片的日子</span></div><div><strong>${Object.keys(d.moodCounts).length}</strong><span>出现过的心情</span></div></div><div class="annual-section"><div class="annual-section-head"><h3>一年里的节奏</h3><span>每个月的记录密度</span></div><div class="annual-month-grid">${d.monthStats.map(m=>`<button class="annual-month-card ${m.days?'has-data':''}" data-month="${m.month}"><strong>${m.month+1}月</strong><div class="annual-bar"><i style="height:${Math.min(100,8+m.days*9)}%"></i></div><span>${m.days} 天</span></button>`).join('')}</div></div><div class="annual-bottom-grid"><div class="annual-section"><div class="annual-section-head"><h3>最常出现</h3><span>贴纸</span></div><div class="annual-sticker-list">${d.topStickers.length?d.topStickers.map(x=>`<span>${svgSticker(x.sticker,34)}<b>${escapeHtml(x.sticker.name)}</b><em>${x.n}</em></span>`).join(''):'<span class="tool-muted">还没有足够的数据。</span>'}</div></div><div class="annual-section"><div class="annual-section-head"><h3>这一年的心情</h3><span>分布</span></div><div class="annual-mood-list">${MOODS.map(m=>{const n=d.moodCounts[m.key]||0;return `<div><span>${escapeHtml(m.label)}</span><i><b style="width:${d.entries.length?Math.round(n/d.entries.length*100):0}%"></b></i><em>${n}</em></div>`;}).join('')}</div></div></div></section>`;document.getElementById('annualBackdrop').addEventListener('click',()=>root.innerHTML='');document.getElementById('closeAnnual').addEventListener('click',()=>root.innerHTML='');document.getElementById('exportAnnualPng').addEventListener('click',async()=>{try{await exportAnnualReviewPng(d);toast('年度回顾 PNG 已导出。');}catch{toast('年度回顾导出失败。');}});document.getElementById('exportAnnualJson').addEventListener('click',()=>exportAnnualJson(d));root.querySelectorAll('.annual-month-card').forEach(btn=>btn.addEventListener('click',async()=>{state.month=Number(btn.dataset.month);state.selectedDay=1;state.viewMode='month';localStorage.setItem('in-days:view','month');root.innerHTML='';renderCalendar();await loadCurrentMonth();await loadSelectedEntry();}));}catch(e){root.innerHTML=`<button class="auth-backdrop" id="annualErrorBackdrop"></button><section class="annual-modal"><button class="close-button" id="closeAnnualError">×</button><div class="section-kicker">YEAR REVIEW</div><h2>年度回顾暂时无法生成</h2><p>${escapeHtml(e.message||'请稍后重试。')}</p></section>`;document.getElementById('closeAnnualError')?.addEventListener('click',()=>root.innerHTML='');}}
function buildAnnualReviewSvg(d){const W=1600,H=1100,bg='#fbfaf6',ink='#403f3a',muted='#969189',line='#e4dfd4';let s=[`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">`,`<rect width="100%" height="100%" fill="${bg}"/>`,`<rect x="54" y="54" width="1492" height="992" rx="42" fill="#fffefb" stroke="${line}"/>`,`<text x="100" y="120" font-size="18" fill="${muted}" letter-spacing="5">IN DAYS · YEAR REVIEW</text>`,`<text x="100" y="198" font-size="66" fill="${ink}">${state.year}</text>`,`<text x="100" y="242" font-size="17" fill="${muted}">${escapeHtml(annualSummarySentence(d))}</text>`];const max=Math.max(1,...d.monthStats.map(m=>m.days));d.monthStats.forEach((m,i)=>{const x=100+(i%6)*235,y=320+Math.floor(i/6)*220,barH=110*(m.days/max);s.push(`<g><text x="${x}" y="${y}" font-size="18" fill="${ink}">${m.month+1}月</text><rect x="${x}" y="${y+22}" width="160" height="110" rx="12" fill="#f1eee7"/><rect x="${x}" y="${y+132-barH}" width="160" height="${barH}" rx="12" fill="#c8b79a"/><text x="${x}" y="${y+162}" font-size="14" fill="${muted}">${m.days} 天 · ${m.stickers} 枚贴纸</text></g>`);});const mood=d.topMood?MOODS.find(m=>m.key===d.topMood[0])?.label:'还没设置心情';s.push(`<text x="1020" y="792" font-size="17" fill="${muted}">这一年的你</text><text x="1020" y="842" font-size="32" fill="${ink}">${escapeHtml(mood)} · ${d.entries.length} 个小片刻</text><text x="1020" y="900" font-size="14" fill="${muted}">最常记录：${escapeHtml(d.topStickers[0]?.sticker?.name||'生活本身')}</text><text x="100" y="1010" font-size="13" fill="${muted}">一隅 · IN DAYS</text></svg>`);return s.join('');}
async function exportAnnualReviewPng(d){await svgToPng(buildAnnualReviewSvg(d),1600,1100,`in-days-${state.year}-year-review.png`);}
function exportAnnualJson(d){const payload={product:'IN DAYS',year:state.year,exported_at:new Date().toISOString(),summary:{recordedDays:d.entries.length,totalStickers:d.entries.reduce((n,e)=>n+(e.stickers||[]).length,0),moodCounts:d.moodCounts},days:d.entries.map(e=>({date:e.entry_date,design:serializeForStorage(e)}))};downloadBlob(new Blob([JSON.stringify(payload,null,2)],{type:'application/json;charset=utf-8'}),`in-days-${state.year}-annual.json`);toast('年度数据 JSON 已导出。');}
...
function monthExportDimensions(onlyRecorded=false){const total=new Date(state.year,state.month+1,0).getDate(),first=(new Date(state.year,state.month,1).getDay()+6)%7;const count=onlyRecorded?Object.values(state.monthEntries).filter(hasEntryContent).length:(first+total);const rows=Math.max(1,Math.ceil(count/7));return {W:1680,H:Math.max(1320,220+rows*(157.5+24)+80)};}
function buildMonthSvg({onlyRecorded=false}={}){
  const {W,H}=monthExportDimensions(onlyRecorded),cols=7,cardW=210,cardH=157.5,gapX=24,gapY=24,left=64,top=220;
  const total=new Date(state.year,state.month+1,0).getDate(),first=(new Date(state.year,state.month,1).getDay()+6)%7;
  let dates=[];for(let d=1;d<=total;d++){const key=dateKey(state.year,state.month,d),e=state.monthEntries[key];if(!onlyRecorded||hasEntryContent(e))dates.push(d);}
  const outH=H;
  let s=[`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${outH}" viewBox="0 0 ${W} ${outH}">`,`<rect width="100%" height="100%" fill="#fbfaf6"/>`,`<text x="64" y="70" font-size="18" fill="#9b9991" letter-spacing="4">${state.year}</text>`,`<text x="64" y="148" font-size="72" fill="#3f403b" font-family="Arial, sans-serif">${escapeHtml(monthNames[state.month])}</text>`];
  if(onlyRecorded)s.push(`<text x="64" y="184" font-size="14" fill="#9b9991">仅有记录的日子</text>`);
  dates.forEach((d,idx)=>{const key=dateKey(state.year,state.month,d),e=state.monthEntries[key],idx2=onlyRecorded?idx:first+d-1,col=idx2%7,row=Math.floor(idx2/7),x=left+col*(cardW+gapX),y=top+row*(cardH+gapY),bg=e?.background||'#fffdf7';s.push(`<rect x="${x}" y="${y}" width="${cardW}" height="${cardH}" rx="22" fill="${escapeHtml(bg)}" stroke="#e8e4da"/>`);if(e&&hasEntryContent(e)){s.push(`<g transform="translate(${x} ${y}) scale(${cardW/CANVAS_W})">${compositionElementsSvg(e,CANVAS_W,CANVAS_H,{includeDate:false})}</g>`);}s.push(`<text x="${x+16}" y="${y+25}" font-size="13" fill="#9b9991">${String(d).padStart(2,'0')}</text>`);const holiday=holidayCueForDate(key);if(holiday)s.push(`<text x="${x+cardW-16}" y="${y+25}" font-size="8" fill="#d1b46c" text-anchor="end">${escapeHtml(holiday.name.replace('节',''))}</text>`);});
  s.push('</svg>');return s.join('');
}

async function exportMonthPng({onlyRecorded=false}={}){const {W,H}=monthExportDimensions(onlyRecorded);await svgToPng(buildMonthSvg({onlyRecorded}),W,H,`in-days-${state.year}-${String(state.month+1).padStart(2,'0')}${onlyRecorded?'-records':''}.png`);toast(onlyRecorded?'有记录的日子已导出。':'本月完整月历已导出。');}
function openMonthExport(){
  const root=document.getElementById('exportMonthRoot');if(!root)return;
  root.innerHTML=`<button class="export-backdrop" id="exportBackdrop"></button><section class="export-modal" role="dialog" aria-modal="true"><button class="close-button" id="closeMonthExport">×</button><div class="section-kicker">EXPORT MONTH</div><h2>导出本月</h2><p>选择一种你想留下的格式，不会一次导出全部内容。</p><div class="export-choice-list"><button class="export-choice" id="exportMonthFull"><strong>完整月历 PNG</strong><span>保留整个${monthCN[state.month]}的日期布局与已记录内容。</span></button><button class="export-choice" id="exportMonthRecords"><strong>仅有记录的日子 PNG</strong><span>只整理本月真正留下内容的日期。</span></button><button class="export-choice" id="exportMonthData"><strong>本月数据 JSON</strong><span>导出文字、贴纸位置、样式等可恢复数据。</span></button><button class="export-choice" id="exportAnnualReview"><strong>年度回顾 PNG</strong><span>把当前年份的记录节奏、心情与贴纸整理成一张年度回顾图。</span></button><button class="export-choice" id="exportAnnualData"><strong>年度数据 JSON</strong><span>导出当前年份全部记录，便于长期保存与迁移。</span></button></div></section>`;
  ...
  document.getElementById('exportAnnualReview').addEventListener('click',async()=>{closeMonthExport();try{const d=annualReviewData(await getYearEntries(state.year));await exportAnnualReviewPng(d);toast('年度回顾 PNG 已导出。');}catch(e){toast(e.message||'年度回顾导出失败。');}});
  document.getElementById('exportAnnualData').addEventListener('click',async()=>{closeMonthExport();try{const d=annualReviewData(await getYearEntries(state.year));exportAnnualJson(d);}catch(e){toast(e.message||'年度数据导出失败。');}});
}
